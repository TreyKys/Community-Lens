...
function VerificationTerminal({ topic, onMintingComplete }) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-center text-emerald-500">{topic}</h2>
      <div className="grid grid-cols-2 gap-6">
        {/* Suspect Column */}
        <div className="bg-slate-800 p-4 rounded-lg">
          <h3 className="text-lg font-bold mb-2">Grokipedia Source</h3>
          <button onClick={fetchGrok} className="bg-emerald-500 px-4 py-2 rounded-lg hover:bg-emerald-600">
            ⚡ Fetch from xAI
          </button>
          {manualMode ? (
            <textarea
        ...
        <div className="bg-slate-800 p-4 rounded-lg">
          ...
          <div className="flex space-x-2 mb-2">
            <button onClick={() => setSource('wikipedia')} className={`px-4 py-2 rounded-lg ${source === 'wikipedia' ? 'bg-emerald-500' : 'bg-slate-700'}`}>Wikipedia</button>
            <button onClick={() => setSource('pubmed')} className={`px-4 py-2 rounded-lg ${source === 'pubmed' ? 'bg-emerald-500' : 'bg-slate-700'}`}>PubMed</button>
          </div>
          <button onClick={fetchConsensus} className="bg-emerald-500 px-4 py-2 rounded-lg hover:bg-emerald-600">
            🔍 Fetch Consensus
          </button>
      ...
      <div className="text-center">
        <button onClick={runAnalysis} className="bg-emerald-500 px-6 py-3 rounded-lg text-xl hover:bg-emerald-600">
          RUN ANALYSIS
        </button>
      </div>
