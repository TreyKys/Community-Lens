...
const MarketCard = React.memo(function MarketCard({
  ...
  const marketEqual = Object.keys(next.market).every((key) => {
    const k = key as keyof Market;
    if (k === 'options') {
      return prev.market.options.length === next.market.options.length &&
             prev.market.options.every((opt, i) => opt === next.market.options[i]);
    }
    return prev.market[k] === next.market[k];
  });
