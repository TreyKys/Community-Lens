...
export function PopularMarketsScroll() {
    ...
    return (
      <div className="space-y-2 w-full min-w-0">
        ...
        <div className="flex gap-3 overflow-hidden">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-32 w-[78vw] max-w-[18rem] md:w-72 shrink-0 rounded-xl shimmer" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    // min-w-0 + w-full prevent this block from forcing the page wider when
    // children overflow horizontally — the scroll lives entirely inside.
    <div className="space-y-2 w-full min-w-0">
        ...
        {markets.map(m => {
          ...
          return (
            <button
              ...
              className="snap-start shrink-0 w-[78vw] max-w-[18rem] md:w-72 text-left p-3 rounded-xl border border-border/60 bg-card hover:border-foreground/30 hover:shadow-lg transition-all group active:scale-[0.98]"
            >
              <div className="flex items-center justify-between mb-2 gap-2">
                <Badge className={cn('text-[10px] border-0 truncate', accent.bg, accent.text)}>{accent.label}</Badge>
                {isOpen && (
