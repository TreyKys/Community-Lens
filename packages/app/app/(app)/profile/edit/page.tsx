...
function AvatarSVG({ id, size = 40, selected = false }: { id: number; size?: number; selected?: boolean }) {
  ...
  const gradId = `edit_av${id}_${size}`;
  return (
    <div className={cn('rounded-xl cursor-pointer transition-all duration-150 hover:scale-110', selected && 'ring-2 ring-primary scale-105')}>
...
export default function ProfileEditPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('users')
      .select('username, avatar_id, first_name, dob')
      .eq('id', session.user.id)
      .single()
      .then(({ data }) => {
        if (data) {
          setProfile(data);
          setSelectedAvatar(data.avatar_id ?? 0);
  ...
  useEffect(() => {
    ...
    if (username === profile?.username) { setUsernameStatus('unchanged'); return; }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (usernameStatus === 'taken' || usernameStatus === 'invalid') {
      toast({ title: 'Fix the username error first', variant: 'destructive' });
      return;
    }
  ...
  const usernameHint = {
    idle: null,
    unchanged: <span className="text-muted-foreground text-xs">Current username</span>,
  ...
  if (!session) {
    return <div className="flex items-center justify-center min-h-[60vh]"><p className="text-muted-foreground">Sign in to edit your profile</p></div>;
  }

  return (
    <div className="max-w-xl mx-auto p-4 sm:p-8 pb-24">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div>
            <h2 className="font-semibold text-sm mb-3">Avatar</h2>
            <div className="flex justify-center mb-4">
              <AvatarSVG id={selectedAvatar} size={64} selected />
            </div>
            <div className="grid grid-cols-8 gap-1.5 max-h-64 overflow-y-auto pr-1">
              {Array.from({ length: 50 }, (_, i) => (
                <div key={i} onClick={() => setSelectedAvatar(i)}>
                  <AvatarSVG id={i} size={32} selected={selectedAvatar === i} />
                </div>
              ))}
            </div>
          </div>
