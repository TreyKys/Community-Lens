-- Phase 3b: atomic locked-odds bet placement.
--
-- Mirrors the discipline of place_bet (parimutuel): one row-locked
-- transaction, no read-then-write race window, single deterministic
-- failure path per error. Adds the new locked-odds bookkeeping on top:
-- freezes locked_odds + vig_at_stake_pct on the bet row, updates the
-- per-market liability ledger, and logs the tier-routing decision.
--
-- Per-market liability ceiling is hardcoded at ₦40,000/side for the
-- launch. Phase 6 will make it reserve-tied and add silent fallback
-- to parimutuel when the ceiling is breached. For now we reject so the
-- behaviour is fully deterministic during pilot.
--
-- This RPC is ONLY called by the /api/bet route when the target
-- market has is_locked_odds = true. The existing place_bet RPC keeps
-- handling every other market untouched.
...
DECLARE
  ...
  -- Per-market liability ceiling. ₦40,000/side at launch. Phase 6
  -- ties this to reserve health.
...
BEGIN
  ...
  IF NOT COALESCE(v_market.is_locked_odds, false) THEN
    -- Caller routed a parimutuel market here by mistake. Surface so
    -- the API route can fall back to place_bet rather than silently
    -- mispricing.
  ...
  -- Accuracy tier column doesn't exist yet (Phase 7 will add it).
  -- Pass NULL so calculate_locked_odds_sql treats every user as
  -- 'rookie' (no discount).
  ...
  -- The view always returns one row, but defend against a future
  -- regression that deletes the singleton.
  IF v_deployable IS NULL OR v_floor IS NULL THEN
    RAISE EXCEPTION 'reserve_unavailable' USING ERRCODE = 'P0003';
  END IF;

  -- ── Pool extraction (jsonb → numeric[], 1-indexed by SQL convention) ──
  ...
  -- ── Compute locked odds via the canonical SQL twin ──────────────
  SELECT *
    INTO v_calc
  ...
  -- ── Liability ceiling check ─────────────────────────────────────
  -- We check BEFORE writing anything so a breach is a clean,
  -- atomic rejection. The check is "would this bet push exposure
  -- on the chosen side over the cap?" — the worst-case-if-this-side-
  -- wins is what we owe.
  ...
  IF v_new_outcome_exposure > c_per_side_liability_cap THEN
    -- Log the routing-deferral attempt before raising so we can
    -- audit "how often does the cap bite?" out of band.
  ...
  v_net_stake  := v_calc.net_stake;   -- same value, kept from the calc row
  ...
  -- ── Insert the bet row with the new locked-odds fields ──────────
  ...
  -- ── Update market pool (same shape as place_bet, so admin
  --     dashboards and resolve_due continue to read it identically) ──
  ...
  -- Recompute worst-case across all outcomes (new chosen-side
  -- exposure vs. existing other-side exposures).
  ...
  -- ── Treasury log — entry rake (same type/shape as place_bet) ────
  ...
  -- ── Volume points (same formula as place_bet) ───────────────────
...
COMMENT ON FUNCTION public.place_bet_locked IS
  'Atomic locked-odds bet placement. Mirror of place_bet (parimutuel) for is_locked_odds markets. Freezes locked_odds + vig_at_stake_pct on the bet row, updates market_liability, logs the routing decision. Called by /api/bet when markets.is_locked_odds = true.';
