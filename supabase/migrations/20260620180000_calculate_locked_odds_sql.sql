-- Phase 3a: PL/pgSQL port of the canonical locked-odds algorithm.
--
-- This is the SQL twin of lib/lockedOdds.ts. Settlement RPCs and
-- atomic bet-placement RPCs call THIS function (inside their own
-- transactions, after row-locking the market) so the odds the user
-- sees in the modal (computed in TS) and the odds we freeze on the
-- bet row (computed in SQL) are produced by mathematically identical
-- functions on identical inputs.
--
-- A parity test (lib/lockedOdds.parity.test.ts) calls both
-- implementations on dozens of input combinations and asserts EXACT
-- equality on the financial outputs (locked_odds, vig_applied, tier,
-- floor_payout, upper_payout). If the two ever drift, that test
-- fails loudly before the drift can land in production.
--
-- Pure / IMMUTABLE — does not read or write any table. Caller is
-- responsible for snapshotting market state, reserve state, and user
-- accuracy tier, and passing them in. This is what keeps the
-- function trivially testable and lets us swap it out without a
...
BEGIN
  ...
  -- 5c. Large-stake surcharge — stake > 50% of opposing effective.
  ...
  FOR i IN 1..v_num_outcomes LOOP
    ...
    END IF;
  END LOOP;
  v_large_stake_surcharge := CASE
    WHEN v_opposing_effective > 0 AND p_stake > v_opposing_effective * 0.5 THEN 0.01
    ELSE 0
  END;
...
COMMENT ON FUNCTION public.calculate_locked_odds_sql IS
  'PL/pgSQL twin of lib/lockedOdds.ts. Pure, IMMUTABLE, parallel-safe. Called by place_bet_locked and the locked-odds settlement path so the odds frozen on the bet row are computed by mathematically identical code to what the user saw in the modal. Parity is enforced by lib/lockedOdds.parity.test.ts.';
