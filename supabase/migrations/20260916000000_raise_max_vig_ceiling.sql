-- Raise the locked-odds vig ceiling from 0.15 to 0.55.
--
-- The operator's explicit decision, after hitting the old ceiling directly:
-- a market was configured aiming for odds of 1.67 and 1.10 on its two
-- outcomes. That pair is mathematically incompatible with a 15% ceiling —
-- 1/1.67 + 1/1.10 = 1.508, so honouring it needs vig >= ~0.508 (see the
-- worked derivation below) — so what actually shipped came out around
-- 2.10/1.33 instead, the loosest (most user-favourable) pair the old
-- ceiling allowed. 0.55 gives ~4 points of headroom above that 0.508
-- minimum, enough to land on 1.67/1.10 (or close variants of it) after
-- accounting for the real stake included in the price, not just the
-- zero-stake theoretical split.
--
--   odds_A = (1/p_A) / (1+vig), odds_B = (1/p_B) / (1+vig), p_A+p_B = 1
--   => 1+vig = 1/odds_A + 1/odds_B  (summing both, since p_A+p_B=1)
--   => vig   = (1/odds_A + 1/odds_B) - 1
--
-- Raising this makes the house's edge bigger and payouts smaller for any
-- market that runs anywhere near the new ceiling — a product decision
-- with a real user-facing cost, not a bug fix. Changing it again should
-- get the same deliberate treatment: confirm the exact pair being
-- targeted, compute the vig that pair actually needs, and only move the
-- ceiling as far as that requires.
--
-- Three places had to move together, on purpose — they're independent
-- copies of the same rule, and that independence (not this ceiling
-- itself) is what let production silently diverge from what was
-- intended in the first place:
--   1. markets.vig_pct CHECK constraint (this migration)
--   2. market_templates.vig_pct CHECK constraint (this migration)
--   3. calculate_locked_odds_sql's c_max_vig (this migration, full
--      function re-emitted from the current live version — CREATE OR
--      REPLACE from anything OTHER than the current version is exactly
--      the mistake that silently deleted the dynamic stake cap in
--      20260630200000 and the multiplier min-odds fix in the same
--      migration; both had to be separately restored later)
-- A fourth, lib/lockedOdds.ts's MAX_VIG, moves in the same commit as
-- this migration — see that file for its own copy of this note.

DO $$
BEGIN
  ALTER TABLE public.markets DROP CONSTRAINT IF EXISTS markets_vig_pct_check;
EXCEPTION WHEN undefined_object THEN NULL;
END $$;

ALTER TABLE public.markets
  ADD CONSTRAINT markets_vig_pct_check CHECK (vig_pct >= 0.04 AND vig_pct <= 0.55);

DO $$
BEGIN
  ALTER TABLE public.market_templates DROP CONSTRAINT IF EXISTS market_templates_vig_pct_check;
EXCEPTION WHEN undefined_object THEN NULL;
END $$;

ALTER TABLE public.market_templates
  ADD CONSTRAINT market_templates_vig_pct_check
  CHECK (vig_pct IS NULL OR (vig_pct >= 0.04 AND vig_pct <= 0.55));

-- calculate_locked_odds_sql, byte-identical to the live version in
-- 20260710000000_smooth_large_stake_surcharge.sql except c_max_vig
-- (0.15 -> 0.55) and this comment. Every other constant, the vig table,
-- both surcharge ramps, the clamp order and the rounding are untouched.
...
DECLARE
  ...
  c_max_vig                   constant numeric := 0.55;
...
BEGIN
  ...
  -- hard step at the 50% threshold (see 20260710000000's header for why
  -- the step was a bug). Mirrors lib/lockedOdds.ts exactly.
